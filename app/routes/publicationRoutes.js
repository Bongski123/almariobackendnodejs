...
router.post('/PubRegister', async (req, res) =>{
    ...
    try {
        ...
        const { title ,authors,abstract, project_id} = req.body;
    
        const insertPublicationQuery = 'INSERT INTO publication ( title, authors,abstract,project_id ) VALUES (?,?,?,?)';
        await db.promise().execute(insertPublicationQuery,[ title, authors,abstract,project_id]);
