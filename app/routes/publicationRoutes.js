...
router.post('/PubRegister', async (req, res) =>{
    ...
    try {
        ...
        const { title ,authors,abstract,publication_type,user_id, project_id} = req.body;
    
        const insertPublicationQuery = 'INSERT INTO publication ( title, authors,abstract,publication_type,user_id,project_id ) VALUES (?,?,?,?,?,?)';
        await db.promise().execute(insertPublicationQuery,[ title, authors,abstract,publication_type,user_id,project_id]);
